...
include("getTriggersSTALTA.jl")
include("filterData.jl")
include("trig2detection.jl")
include("util.jl")

function stalta(dir::String,stas::Array{String},chans::Array{String},fs::Int=100,longWinLength::Float64=60.,shortWinLength::Float64=5.,
    threshold::Float64=3.,overlap::Float64=30.,filterType::String="none",freq::Array{Float64,1}=1,corners::Int=4,dates::Array{String}=[""],outName::String="")
	...
	if occursin("SAC",filename)
        	data = SeisIO.read_data("sac",filename)
	else occursin("MSEED",filename)
		data = SeisIO.read_data("mseed",filename)
	end
        ...
        if isempty(data) == false
            ...
            #get staLta triggers
            triggers = getTriggersSTALTA(trace,fs,longWinLength,shortWinLength,threshold,overlap)
    ...
    #override above if date vectorp is provided manually
    if dates[1] != ""
	...
	if outName != ""
        	...
        	open(outName*".txt","a") do x
        	    writedlm(x,unixDetections)
        	end

        	#open file for writing 
        	open(outName*"Quality.txt","a") do x
        	    writedlm(x,quality)
        	end
	else
        	#open file for writing detections
