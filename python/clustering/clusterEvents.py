...
if readObspy:
    waveforms = obspy.read(templatePath + 'short_waveforms_' + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + 'Hz.h5')
    ...
    waveFile = h5py.File(templatePath +"short_waveform_matrix_" + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + "Hz.h5",'w')
...
# save result
outFile = h5py.File(templatePath + "clustering/" + str(numCluster) + "_cluster_predictions_" + str(prefiltFreq[0]) + "-" + str(prefiltFreq[1]) + "Hz.h5","w")
...
for c in range(numCluster):
    ...
    #plt.show()
    plt.savefig(templatePath + "clustering/" + str(numCluster) + "_cluster_clust" + str(c) + ".png")
